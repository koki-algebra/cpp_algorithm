...
「区間 $[l, r)$ に $x$ を加算する」というクエリが $Q$ 個与えられ、最終的な各要素の値を知りたい。
毎回区間全体を書き換えると $O(NQ)$ でTLEするが、**差分配列**に対して2点だけ操作し、最後にまとめて累積和を取れば $O(N + Q)$ で処理できる。

累積和が「区間の合計を高速に求める」テクニックなのに対し、いもす法は「区間への加算を高速に反映する」逆向きのテクニックとみなせる。

#### 仕組み

区間 `[l, r)` に `x` を加算したいとき、差分配列 `diff` に対して

- `diff[l] += x`（`l` から先、加算量が `+x` される）
- `diff[r] -= x`（`r` から先、その `+x` を打ち消す）

の2点操作だけを行う。全クエリを処理した後に `diff` の累積和を取ると、各位置の最終的な加算値が求まる。

```
n=5, クエリ [1,4) に +3 を加算

diff: index   0  1  2  3  4  5
      value   0 +3  0  0 -3  0   ← diff[1]+=3, diff[4]-=3

a:    index   0  1  2  3  4
      value   0  3  3  3  0      ← 累積和
```

#### 実装（0-indexed半開区間 `[l, r)` 版）

```cpp
ll n, q;
cin >> n >> q;

vector<ll> diff(n + 1, 0);  // 差分配列。番兵として +1 のサイズを確保

rep(i, q) {
    ll l, r, x;
    cin >> l >> r >> x;  // 区間 [l, r) に x を加算
    diff[l] += x;
    diff[r] -= x;
}
...
- `diff` のサイズを `n + 1` にしているのは、クエリの `r` が `n`（配列の末尾ちょうど）になっても `diff[r] -= x` が配列外アクセスにならないようにするための番兵。`diff[n]` は書き込まれることはあっても、累積和の計算では読まれないため答えには影響しない。

#### 入力が1-indexed閉区間 `[l, r]` の場合

AtCoderなどでは「`l` 番目から `r` 番目まで（両端含む）」という1-indexed閉区間で与えられることが多い。この場合、**`l` だけ `-1` して `r` はそのまま**使う（1-indexed閉区間 `[l, r]` は0-indexed半開区間 `[l-1, r)` に等しいため）。

```cpp
rep(i, q) {
    ll l, r, x;
    cin >> l >> r >> x;  // 1-indexed閉区間 [l, r] に x を加算
    diff[l - 1] += x;    // l のみ 0-indexed に変換
    diff[r] -= x;         // r はそのまま
}
```

`l--; r--;` のように両方をずらすと、区間の最後の要素が加算対象から漏れてしまうので注意。
...
| いもす法 | 区間への加算を何度も行う | クエリ $O(1)$, 復元 $O(N)$ |
...
| [ABC014D - 彼女のパートナーを探せ](https://atcoder.jp/contests/abc014/tasks/abc014_4) | いもす法（区間加算の典型） | ★★☆ |
